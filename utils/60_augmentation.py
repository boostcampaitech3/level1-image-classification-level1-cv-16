...
df = pd.read_csv('/opt/ml/input/data_copy/train/train.csv')
pat = '/opt/ml/input/data_copy/train/images/' # 기존 image 폴더를 복사한 폴더 경로
df_test = pd.read_csv('/opt/ml/input/data_copy/train/train.csv') # 기존 train.csv를 복사한 경로
...
for i in range(len(df)):
    if df['age'][i] >= 60:
        ...
        update.to_csv('/opt/ml/input/data_copy/train/test_train.csv', header=True, index=True)
